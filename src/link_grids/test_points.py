...
if __name__ == "__main__":
    ...
    handles = []
    all_points = []
    links = "r_elbow_flex_link r_forearm_link r_forearm_roll_link r_gripper_l_finger_link r_gripper_l_finger_tip_link r_gripper_palm_link r_gripper_r_finger_link r_gripper_r_finger_tip_link r_upper_arm_link r_wrist_flex_link r_wrist_roll_link".split()
    # links = ["r_upper_arm_link"]
    point_dict = {}
    #### YOUR CODE HERE ####
    with env:
        ...
        for link in links:
            points = np.genfromtxt("./pr2_arm_only/"+link+".csv", delimiter=",")
            T_link = robot.GetLink(link).GetTransform()
            new_points = points + T_link[:3,3]
            # new_points = []
            # for p in points:
            #     print p[0]
            #     if p[0] > 0.1:
            ...
            point_dict[link] = new_points


    for link in point_dict:
        handles.append(env.plot3(points=np.array(point_dict[link]), 
           pointsize= 1.0, 
           colors=array(((0,0,1))), 
           drawstyle=0))
        print link
